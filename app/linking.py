...
def features(source: dict, target: dict) -> dict:
    ...
    return {
        ...
        "period": 0.5 if not hits else float(any(
...
def link(candidates: dict, scorer: Scorer, threshold: float = THRESHOLD) -> list[dict]:
    ...
    for source in candidates["sources"]:
        model_scores = scorer.score(source, targets)
        ...
        for target, model, rule in zip(targets, model_scores, rule_scores):
            parts = {
                ...
                "upstream": round(min(source["confidence"], target["confidence"]), 3),
            }
        ...
        # One link per summary row. A near-tie at the top is reported as such
        # rather than guessed, and the runners-up stay visible either way.
        if best < threshold:
            status = "unlinked"
...
def run(candidates: dict, scorer: Scorer, out_dir: Path, threshold: float = THRESHOLD) -> list[dict]:
